  ...
  return (
    <div className="card-surface grid grid-cols-2 gap-1 p-2 sm:grid-cols-3">
      {items.map(({ key, label, value, icon: Icon }) => (
        <div key={key} className="flex items-center gap-3.5 rounded-lg p-4 transition-colors hover:bg-surface-elevated/60">
          <Icon className="shrink-0 text-text-muted" width={20} height={20} />
